...
- **Timeline entry edits/deletes:** Home + History long-press cards now use session context mutation APIs (`editTimelineEntryTime`, `deleteTimelineEntry`) so timers/timeline recalculate from updated events. Deleting a pause/resume entry also removes its matched counterpart (pair inferred by ordered pause stack).
- **Timeline edit safety:** event time edits are sequence-validated in session mutation helpers (must stay between previous/next entries and keep legal phase order: `clock_in -> pause_started -> pause_ended -> clock_out`), and edit modal surfaces validation errors instead of persisting invalid order.
...
- **Reanimated 4 + Worklets:** for React Native Community CLI, [`babel.config.js`](../babel.config.js) must include `'react-native-worklets/plugin'` and it must be the **last** Babel plugin ([Reanimated getting started](https://docs.swmansion.com/react-native-reanimated/docs/fundamentals/getting-started/#react-native-community-cli), [Worklets docs](https://docs.swmansion.com/react-native-worklets/docs/#react-native-community-cli)).
- **Motion presets:** shared durations/easing/layout transition helpers live in [`src/theme/motion.ts`](../src/theme/motion.ts); keep frequent interactions in micro/standard ranges and animate mostly `opacity` + `transform`.
- **Reduced motion:** use Reanimated `useReducedMotion()` and feed timing/spring configs through shared helpers so animation intensity is lowered consistently when accessibility prefers less motion.
- **List/screen motion:** prefer Reanimated layout/list transitions (`itemLayoutAnimation`, entering/exiting builders) over JS-driven per-frame effects for smoother Fabric performance and lower battery cost ([Reanimated performance guide](https://docs.swmansion.com/react-native-reanimated/docs/guides/performance), [layout animations](https://docs.swmansion.com/react-native-reanimated/docs/category/layout-animations)).
- **Cross-fading conditional Animated.View siblings:** Reanimated keeps an exiting view in the layout for the duration of `exiting` while the new branch is already mounted, so two sibling `Animated.View`s in a column with `gap` will visibly stack mid-transition. Wrap them in a `position: 'relative'` container with a `minHeight` matching the children, and give each branch `position: 'absolute'` so they cross-fade in place (see `CheckInStatusCard` `actionsContainer` / `actionsLayer`). Reanimated [entering/exiting docs](https://docs.swmansion.com/react-native-reanimated/docs/layout-animations/entering-exiting-animations).
- **Leave date pick:** [`react-native-calendars`](https://github.com/wix/react-native-calendars) — `Calendar` on Leaves **Add leave** modal; pass `theme` for dark surfaces (`calendarBackground`, `dayTextColor`, `selectedDayBackgroundColor`, etc.).
- **Leaves balance policy:** [`src/pages/Leaves/leaveBalanceMath.ts`](../src/pages/Leaves/leaveBalanceMath.ts) is the source of truth: `full_day=1`, any half-day=`0.5`, remaining can be negative, monthly accrual is `+1.5` from the current month onward (no backfill), and modal save sets an override baseline while enforcing used `>=` entry-derived usage. Same-day entry normalization in [`src/pages/Leaves/index.tsx`](../src/pages/Leaves/index.tsx): if both half-days exist for a day they collapse to one `full_day`; duplicate same-day entries are not retained.
- **Leaves persistence:** [`src/pages/Leaves/leaveStorage.ts`](../src/pages/Leaves/leaveStorage.ts) stores leave timeline + baseline + accrual start month in AsyncStorage key `worklyn:leaves:v1`; Leaves screen gates edit/add interactions until hydration completes and only then persists updates to avoid replacing restored data on initial mount. Loader canonicalizes baseline at hydrate-time from effective balances (`deriveLeaveBalances`) and resets anchors (`entryUsedAtSet`, `accrualAtSet`) to current totals so reopen is idempotent even if older snapshots were inconsistent ([AsyncStorage docs](https://github.com/react-native-async-storage/async-storage#readme)).
...
- **Tabs + shared chrome:** [`src/navigation/MainShell.tsx`](../src/navigation/MainShell.tsx) — shared header ([`AppHeaderBar`](../src/components/layout/AppHeaderBar/index.tsx)) + display-name modal above a [`createBottomTabNavigator`](https://reactnavigation.org/docs/bottom-tab-navigator) (`Home`, `History`, `Leaves`).
